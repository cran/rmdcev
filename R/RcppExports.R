...
Shuffle_rng <- function(inv_temp, nerrs, base_rng__ = get_rng(), pstream__ = get_stream()) {
...
DrawError_rng <- function(quant_num, quant_j, price_j, psi_j, phi_j, gamma_j, alpha, scale, model_num, nalts, nerrs, cond_error, draw_mlhs, base_rng__ = get_rng(), pstream__ = get_stream()) {
...
SortParmMatrix <- function(MUzero, price, gamma, alpha_phi, nalts, pstream__ = get_stream()) {
...
ComputeUtilJ <- function(income, quant_j, price_j, psi_j, phi_j, gamma_j, alpha, nalts, model_num, pstream__ = get_stream()) {
...
ComputeKtUtilM <- function(M, lambda1, psi, mu, alpha_1, pstream__ = get_stream()) {
    .Call(`_rmdcev_ComputeKtUtilM`, M, lambda1, psi, mu, alpha_1, pstream__)
}

HicksianDemand <- function(util, price, MUzero, phi, gamma, alpha, nalts, algo_gen, model_num, tol_l, max_loop, pstream__ = get_stream()) {
    .Call(`_rmdcev_HicksianDemand`, util, price, MUzero, phi, gamma, alpha, nalts, algo_gen, model_num, tol_l, max_loop, pstream__)
}

CalcWTP_rng <- function(income, quant_j, price, price_p_policy, psi_p_sims, phi_p_sims, psi_sims, phi_sims, gamma_sims, alpha_sims, scale_sims, nerrs, cond_error, draw_mlhs, algo_gen, model_num, price_change_only, tol, max_loop, base_rng__ = get_rng(), pstream__ = get_stream()) {
...
CalcMarshallianDemand_rng <- function(income, quant_j, price, price_p_policy, psi_p_sims, phi_p_sims, psi_sims, phi_sims, gamma_sims, alpha_sims, scale_sims, nerrs, cond_error, draw_mlhs, algo_gen, model_num, price_change_only, tol, max_loop, base_rng__ = get_rng(), pstream__ = get_stream()) {
...
CalcmdemandOne_rng <- function(income, price, psi_j, phi_j, gamma_j, alpha, scale, nerrs, model_num, algo_gen, tol, max_loop, base_rng__ = get_rng(), pstream__ = get_stream()) {
