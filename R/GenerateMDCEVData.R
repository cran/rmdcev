...
GenerateMDCEVData <- function(
	...
	phi_parms = c(-5, 0.5, 2),
	dat_psi_i = matrix(
		2 * stats::runif(nobs * length(psi_i_parms)),
		nobs,
		length(psi_i_parms)
	),
	dat_psi_j = cbind(matrix(
		stats::runif(nalts * (length(psi_j_parms)), 0, 1),
		nrow = nalts
	)),
	dat_phi = cbind(matrix(
		stats::runif(nalts * (length(phi_parms)), 0, 1),
		nrow = nalts
	)),
	nerrs = 1,
	tol = 1e-20,
	...
	if (model != "kt_ee") {
		...
		colnames(dat_phi) <- c(paste(
			rep('phi_', ncol(dat_phi)),
			1:ncol(dat_phi),
			sep = ""
		))
	...
	data <- mdcev.data(data, id.var = "id", alt.var = "alt", choice = "quant")

	out <- list(data = data, parms_true = parms_true)
	return(out)
}
